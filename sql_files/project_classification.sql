-- Database for week project Classification
--
...
-- 2 --
-- Create a table credit_card_data with the same columns as given in the csv file. 
...
select customer_id, credit_rating, credit_cards_held, own_your_home, household_size, offer_accepted
from credit_card_data
where credit_rating in ('Low','Medium','High') 
and offer_accepted = 'Yes' 
and average_balance < 850
#and household_size >= 3
order by offer_accepted DESC;

select customer_id, average_balance, credit_rating, offer_accepted
from credit_card_data
...
select customer_id, nr_of_bank_accounts_open, credit_cards_held from credit_card_data
group by nr_of_bank_accounts_open
order by nr_of_bank_accounts_open;
